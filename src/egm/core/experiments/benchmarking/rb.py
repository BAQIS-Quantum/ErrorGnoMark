...
# =============================================================================
# File    : src/egm/core/experiments/benchmarking/rb.py
# Version : v5.5 - RB / IRB with Optional Randomized Compiling (RC)
# Author  : OpenAI-Assistant
# =============================================================================
"""
Randomized Benchmarking (RB) Experiments - Unified Controller
...
Supports:
    ...
    • Interleaved RB
    • Purity RB (PRB)
    • Optional Randomized Compiling / Pauli Twirling (RB + RC)

Design goals:
    - Backward compatible
    - Minimal intrusion into existing RB logic
    - RC applied only at circuit-generation stage
"""
...
from typing import List, Dict, Optional, Union, Tuple, Protocol, Any, Literal
...
# =============================================================================
# RB Compilation / Noise Tailoring Options
# =============================================================================
@dataclass
class RBCompilationOptions:
    """
    Controls RB-level circuit randomization (e.g. Randomized Compiling).
    """
    scheme: Optional[Literal["RC", "PAULI_TWIRL"]] = None
    pauli_set: Optional[List[str]] = None  # default: ["I","X","Y","Z"]
...
class StandardRBExperiment:
    """
    Standard RB experiment controller with optional Randomized Compiling (RC).
    """
    ...
    def __init__(
        ...
        clifford_factory: Optional[Union[CliffordFactory, str]] = None,
        x_axis_mode: str = "GATE_COUNT",
        rb_compilation: Optional[RBCompilationOptions] = None,
    ):
        self.qubits = [qubits] if isinstance(qubits, int) else qubits
        self.num_qubits = len(self.qubits)
        self.depths = depths or DEFAULT_RB_DEPTHS
        ...
        self.seed = seed
        self.x_axis_mode = x_axis_mode.upper()
        self.rb_compilation = rb_compilation

        if self.x_axis_mode not in ["DEPTH", "GATE_COUNT"]:
            raise ValueError("x_axis_mode must be 'DEPTH' or 'GATE_COUNT'.")

        # Resolve Clifford factory
        if clifford_factory is None:
            clifford_factory = CliffordGateSet()
        elif isinstance(clifford_factory, str):
            clifford_factory = get_gate_set(clifford_factory)
        ...
        logging.info(
            f"[RB-Init] {self.num_qubits}Q RB "
            f"(RC={'ON' if rb_compilation and rb_compilation.scheme else 'OFF'})"
        )

    # ------------------------------------------------------------------
    def _apply_rb_compilation(
        self,
        circ: QuantumCircuit,
        rng: random.Random,
    ):
        """
        Apply RB-level noise tailoring (e.g. RC / Pauli twirling).
        """
        if not self.rb_compilation or not self.rb_compilation.scheme:
            return

        scheme = self.rb_compilation.scheme
        paulis = self.rb_compilation.pauli_set or ["I", "X", "Y", "Z"]

        if scheme in ("RC", "PAULI_TWIRL"):
            for q in self.qubits:
                p = rng.choice(paulis)
                if p != "I":
                    circ.add_gate(Gate(p.lower(), [q]))
    ...
    def _generate_circuit(
        ...
        for _ in range(depth):
            ...
            self._apply_rb_compilation(circ, rng)
            circ.add_gates(fwd)
    ...
    def circuits(self) -> List[QuantumCircuit]:
        rng = random.Random(self.seed)
        return [
            self.generate_single_circuit(d, rng.random())
            for d in self.depths
            for _ in range(self.circuits_per_depth)
        ]
    ...
    def run(
        ...
        logging.info(f"=== Running {rb_type} RB with {len(circuits)} circuits ===")
        ...
        if engine:
            ...
            raise ValueError("Engine execution required.")
        ...
        if plot:
            plot_rb_single(rb_fit, title=f"{rb_type} RB Decay", x_axis_mode=self.x_axis_mode)
            plot_prb_single(prb_fit, num_qubits=self.num_qubits)
...
# =============================================================================
class InterleavedRBExperiment(StandardRBExperiment):
    """Interleaved RB with per-gate EPG and fidelity (supports RB + RC)."""
    ...
    def __init__(
        ...
        clifford_factory: Optional[Union[CliffordFactory, str]] = None,
        x_axis_mode: str = "GATE_COUNT",
        rb_compilation: Optional[RBCompilationOptions] = None,  # ✅ 新增
    ):
        # ---- Clifford factory handling（完全保留你的逻辑）----
        if clifford_factory is None:
            cf = CliffordGateSet()
            cf.two_qubit_gate_name = "cz"
            clifford_factory = cf
        ...
        super().__init__(
            ...
            clifford_factory=clifford_factory,
            x_axis_mode=x_axis_mode,
            rb_compilation=rb_compilation,  # ✅ 透传
        )
        ...
        self._is_clifford_gate = (
            self.num_qubits == 1
            or interleaved_gate.name.lower() in self._CLIFFORD_NAMES
        )
    ...
    def _generate_single_interleaved_circuit(
        ...
        for _ in range(depth):
            ...
            for g in fwd:
                circ.add_gate(g)
                forward_seq.append(g)

            # ---- interleaved target gate ----
            circ.add_gate(self.interleaved_gate)
            forward_seq.append(self.interleaved_gate)

            # ✅ RC after target gate
            self._apply_rb_compilation(circ, rng)

        # ---- append inverse (Clifford only) ----
        if self._is_clifford_gate:
            inv_circ = QuantumCircuit(self.qubits)
            inv_circ.add_gates(forward_seq)
            circ.add_gates(inv_circ.inverse().gates)
    ...
    def run(
        ...
        # ---- auto-generate circuits if needed ----
        if engine and (circuits_ref is None or circuits_int is None):
            ref_exp = StandardRBExperiment(
                self.qubits,
                self.depths,
                ...
                self.clifford_factory,
                self.x_axis_mode,
                rb_compilation=self.rb_compilation,  # ✅ reference 也用同一 RC
            )
            circuits_ref = ref_exp.circuits()
            circuits_int = self.circuits()
            logging.info("[IRB] Auto-generated reference and interleaved circuits.")
        ...
        ref_exp = StandardRBExperiment(
            ...
            self.clifford_factory,
            self.x_axis_mode,
            rb_compilation=self.rb_compilation,  # ✅ RC 一致
        )
        ref_res = ref_exp.run(
            circuits_ref,
            engine,
